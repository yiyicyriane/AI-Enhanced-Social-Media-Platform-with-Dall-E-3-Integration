...
import PhotoAlbum from "react-photo-album"; //图片预览
import Lightbox from "yet-another-react-lightbox"; //点击图片后可以全屏预览

import OpenAI from "openai"; //openai的4o模型
...
//styled-components 是一个用于在 React 应用中编写 CSS 样式的库，它使用 CSS-in-JS 的方式
...
export default function Landing() {
  ...
  const [inputValue, setInputValue] = useState(""); // State to store the input value，保存用户输入的prompt
  const [isGeneratingImage, setIsGeneratingImage] = useState(false);//是否正在生成图片
  const [generatedImageUrl, setGeneratedImageUrl] = useState(); //Dalle3返回的是图片的链接
  ...
  }, [generatedImageUrl]); //每当generate image url变了的时候就运行use effect
  ...
  const handleInputChange = (event) => {//监听用户输入的prompt
    setInputValue(event.target.value);
  };

  const handleUploadImage = async () => {
    //把图片从url下载下来
    //async await是promise的Syntactic Sugar
